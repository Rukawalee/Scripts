...
function toAct(actIn) {
    ...
    toFind(c_className, c_depth, "", c_indexInParent, true).findOne().click();
    subThread.interrupt();
    log("状态：活动入口加载完成");
    log("状态：活动页面加载中");
...
function custom() {
    var is_custom = dialogs.rawInput("是否定制组件信息", "否");
    switch (is_custom) {
        case "是":
